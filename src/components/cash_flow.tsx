...
export default function CashFlows({ cashFlows }) {
  ...
  const oaOption = {
    ...
    legend: {
      data: ['经营收到的现金', '税费返还', '收到其他现金', '支付商品现金', '支付职工现金', '支付税费', '支付其他现金', '现金流量净额']
    },
    grid: {
      left: '3%',
      right: '4%',
  ...
  const iaOption = {
    ...
    legend: {
      data: ['收回投资', '投资收益', '处置固定/无形/长期资产', '处置子公司', '收到其他现金', '购买固定/无形/长期资产', '投资支付', '获取子公司', '支付其他', '投资活动净额']
    },
    grid: {
      left: '3%',
      right: '4%',
  ...
  const faOption = {
    ...
    legend: {
      data: ['吸收投资', '取得借款', '发行债券', '收到其他', '偿还债务', '分配股利、利润或偿付利息', '支付其他', '现金流量净额']
    },
    grid: {
      left: '3%',
      right: '4%',
